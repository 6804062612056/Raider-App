...
class BorderButton(Button):
    ...
    def __init__(self, **kwargs):
        ...
        # กำหนดฟอนต์เป็น Roboto ของ Kivy
        self.font_name = "Roboto"
        self.font_size = "15sp"
        self.bold = True

        # ปิดพื้นหลังเดิมของ Kivy
        self.background_normal = ""
        self.background_down = ""
        self.background_color = (0, 0, 0, 0)
        self.color = (0.12, 0.16, 0.22, 1)  # สีตัวอักษรเข้ม

        # วาดพื้นหลังขอบมน และเส้นขอบ
        with self.canvas.before:
            # 1. สีพื้นหลังปุ่ม
            self.bg_color = Color(1, 0.78, 0, 1)  # สีเหลืองหลัก
            self.bg_rect = RoundedRectangle(
                pos=self.pos, size=self.size, radius=[20]
            )

            # 2. เส้นขอบ
            self.line_color = Color(0.85, 0.85, 0.85, 0.8)
        ...
        # ผูก Event เพื่ออัปเดตตำแหน่ง และเปลี่ยนสีตามการกด
        self.bind(
    ...
    def update_graphics(self, *args):
        """อัปเดตขนาดและตำแหน่งให้ตรงกับ Widget"""
    ...
    def on_state_change(self, instance, value):
        """เปลี่ยนสีเมื่อมีการกดปุ่ม"""
        if value == "down":
            self.bg_color.rgba = (0.9, 0.7, 0, 1)  # สีเหลืองเข้มตอนกด
        else:
            self.bg_color.rgba = (1, 0.78, 0, 1)  # สีเหลืองปกติ
