...
class Home2(Screen):
    ...
    def on_enter(self, *args):
        ...
        if Home2._cached_email and Home2._cached_email == current_email:
            self.role = Home2._cached_role
            self.firstname = Home2._cached_fname
            self.lastname = Home2._cached_lname
            self.money = Home2._cached_money
    ...
    def _refresh_money_async(self, current_email):
        ...
        try:
            ...
            if user_db and "money" in user_db:
                ...
                if money_str != Home2._cached_money:
                    Home2._cached_money = money_str
                    Clock.schedule_once(lambda dt: setattr(self, "money", money_str))
        except Exception as e:
            print("Error refreshing money from DB in Home2:", e)

    def _load_user_data_async(self, current_email):
        """อ่านข้อมูลผู้ใช้จากไฟล์เบื้องหลังเฉพาะตอนเปลี่ยนบัญชี"""
        try:
            ...
            Home2._cached_email = current_email
            Home2._cached_role = role_val
            Home2._cached_fname = fname_val
            Home2._cached_lname = lname_val
            Home2._cached_money = money_str

            # ส่งค่ากลับมาอัปเดตบน Main Thread
            Clock.schedule_once(
                lambda dt: self._update_ui(role_val, fname_val, lname_val, money_str)
            )
        except Exception as e:
            print("Error loading user data in Home2:", e)
