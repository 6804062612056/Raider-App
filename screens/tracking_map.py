...
from kivy.graphics import PopMatrix, PushMatrix, Rotate, Color, RoundedRectangle
...
from functions import file, database, network
...
class TrackingMap(Screen):
    ...
    rider_rating_text = StringProperty("Rider Rating: -")
    bearing = NumericProperty(0)

    PEER_TIMEOUT = 7.0  # ขยายเวลาเผื่อจังหวะมือถือหน่วงเน็ตบน Android[cite: 14]
    ...
    def __init__(self, **kwargs):
        ...
        self.is_peer_disconnected = False
    ...
    def on_enter(self):
        ...
        # เปิด Multicast Lock บน Android ป้องกัน OS บล็อก UDP ในแอปเบื้องหลัง[cite: 14]
        if platform == "android":
        ...
        if self.role == "rider":
            ...
            if not self.periodic_event:
                self.periodic_event = Clock.schedule_interval(
                    self.send_periodic_location, 0.5
                )
        else:
            self.send_route_to_rider()
            self.add_rider_rating_display()
            ...
            if not self.periodic_event:
                self.periodic_event = Clock.schedule_interval(
                    self.customer_heartbeat_check, 0.5
                )
    ...
    def add_finish_button(self):
        ...
        with self.finish_btn.canvas.before:
            self.finish_btn_color = Color(0, 0.75, 0.3, 1)
            self.finish_btn_rect = RoundedRectangle(
                pos=self.finish_btn.pos, size=self.finish_btn.size, radius=[25]
            )

        def update_btn_graphics(instance, *args):
            self.finish_btn_rect.pos = instance.pos
            self.finish_btn_rect.size = instance.size

        def on_btn_state_change(instance, value):
            if value == "down":
                self.finish_btn_color.rgba = (0, 0.6, 0.25, 1)
            else:
                self.finish_btn_color.rgba = (0, 0.75, 0.3, 1)

        self.finish_btn.bind(
            pos=update_btn_graphics,
            size=update_btn_graphics,
            state=on_btn_state_change,
            on_release=self.on_empty_click,
        )
    ...
    def on_empty_click(self, instance):
        ...
        if self.is_peer_disconnected or (time.time() - self.last_peer_active_time > 2.0):
            self.handle_peer_disconnected()
            return
        ...
        def _send_finish():
            try:
                ...
                subnets = network.get_broadcast_subnets()
                for b_ip in subnets:
                    try:
                        sock.sendto(payload, (b_ip, 5001))
                    except Exception:
                        pass
                sock.close()
            except Exception as e:
    ...
    def send_periodic_location(self, dt):
        if self.role == "rider":
            ...
            self.check_peer_timeout()

    def customer_heartbeat_check(self, dt):
        if self.role == "customer":
            self.send_route_to_rider()
            self.send_customer_alive()
            self.check_peer_timeout()

    def send_customer_alive(self):
        def _send():
            try:
                payload = json.dumps({
                    "type": "CUSTOMER_ALIVE",
                    "customer_email": file.read_email()
                }).encode("utf-8")
    ...
    def check_peer_timeout(self):
        if not self.is_peer_disconnected and (time.time() - self.last_peer_active_time > self.PEER_TIMEOUT):
            self.is_peer_disconnected = True
            print("PEER TIMEOUT DETECTED! Cancelling trip...")
            Clock.schedule_once(lambda dt: self.handle_peer_disconnected())
    ...
    def handle_peer_disconnected(self):
        ...
        target_home = "home_customer" if self.role == "customer" else "home_rider"
        self.go_to(target_home)

        show_error_popup(message="Connection lost")
            ...
            try:
                ...
                while self.is_listening:
                    try:
                        ...
                        elif msg_type == "CUSTOMER_ALIVE" and self.role == "rider":
                            self.last_peer_active_time = time.time()
    ...
    def start_gps(self):
        ...
        # 1. ขอพิกัดจาก GPS ปกติ (แม่นยำสูงเมื่ออยู่กลางแจ้ง)[cite: 14]
        ...
        # 2. เพิ่มการขอพิกัดจาก Network Provider (ช่วยให้จับตำแหน่งได้ไวขึ้นผ่าน Wi-Fi/เสาสัญญาณมือถือในตึก)[cite: 14]
